...
from config import LOGS_SERVICES_DIR

logger = logging.getLogger(__file__)
logger.setLevel(logging.INFO)
file_handler = logging.FileHandler(LOGS_SERVICES_DIR, "w")
file_formatter = logging.Formatter("%(asctime)s - %(filename)s -%(funcName)s - %(levelname)s: %(message)s")
file_handler.setFormatter(file_formatter)
logger.addHandler(file_handler)


def searching_by_phone_numbers(operations):
    """
    Функция возвращает JSON со всеми транзакциями, содержащими в описании мобильные номера.
    :param operations: список транзакций
    :return: отфильтрованный json-ответ по номерам телефонов в описании транзакций
    """
    operations_with_phones = []
    pattern = re.compile(r"(\+\d\s\d+\s\d+-\d+-\d+)")
    logger.info("Функция начала работу. Отсеивает транзакции, в описании которых нет номера телефона")
    try:
        for i in operations:
            if re.search(pattern, i.get("Описание")):
                operations_with_phones.append(i)
        logger.info("Функция завершила работу. Возвращает json-ответ")
        result = json.dumps(operations_with_phones, indent=4, ensure_ascii=False)
        return result
...
if __name__ == "__main__":
    ...
    print(searching_by_phone_numbers(op))
